...
<body>
    ...
    # 1. Проверяем что вообще приходит от API
    print("=== ТЕСТ ЗАПРОСА ===")
    response = requests.get(f'{WEBHOOK}crm.type.list')
    print("Статус код:", response.status_code)
    print("Текст ответа:", response.text[:500])  # Первые 500 символов
    print("\n")

    # 2. Пробуем другой метод - профиль
    print("=== ПРОВЕРКА ВЕБХУКА ===")
    profile = requests.get(f'{WEBHOOK}profile')
    print("Профиль статус:", profile.status_code)
    print("Профиль:", profile.json())
    print("\n")

    # 3. Пробуем получить элементы напрямую по ID 142
    print("=== ПОЛУЧАЕМ ЭЛЕМЕНТЫ ПРОЦЕССА 142 ===")
    items = requests.get(f'{WEBHOOK}crm.item.list', params={'entityTypeId': 142})
    print("Статус:", items.status_code)
    st.write("Элементы:", items.json())